...
open class CheckInApi(private val checkInDataManager: CheckInDataManager,
    ...
    open fun performCheckIn(@PathVariable("eventName") eventName: String,
        ...
        return Optional.ofNullable(username)
            .map {
                ResponseEntity.ok(checkInDataManager.performCheckIn(eventName, ticketIdentifier, ticketIdentifier, it!!))
            }.orElseGet {
    ...
    open fun printLabel(@PathVariable("eventName") eventName: String,
        ...
        return Optional.ofNullable(username)
            .map {
                ResponseEntity.ok(checkInDataManager.forcePrintLabel(eventName, ticketIdentifier, ticketIdentifier, it!!))
            }.orElseGet {
                ResponseEntity(HttpStatus.UNAUTHORIZED)
            }
    }
